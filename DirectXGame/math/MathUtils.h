...
inline Matrix4x4 Inverse(const Matrix4x4& m) { return 1.0f / Determinant(m) * Adjugate(m); }
...
inline void Copy(const float in[2], Vector2& out) { memcpy(&out.x, in, sizeof(float) * 2); }
inline void Copy(const float in[3], Vector3& out) { memcpy(&out.x, in, sizeof(float) * 3); }
